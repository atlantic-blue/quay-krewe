Feature: A session is isolated by the runtime the operator chose
  ...
  Docker Desktop. The macOS runtime gives a session a macOS virtual machine, which is the only one of
  these that can build a darwin application. The host backend runs a session on the machine itself
  ...
  internal/sandbox/sandboxtest. Docker runs it in continuous integration. Apple container and the
  macOS runtime run the same cases on a macOS machine that holds the tool.
