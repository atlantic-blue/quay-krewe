Feature: A session is isolated by the runtime the operator chose
  ...
  Docker Desktop. The host backend runs a session on the machine itself with no isolation, and it is a
  stopgap.
  ...
  internal/sandbox/sandboxtest: Docker runs it in continuous integration, and Apple container runs the
  same cases on a macOS machine that holds the tool.
